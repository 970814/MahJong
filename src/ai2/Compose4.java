...
//    }
    ...
    static HashSet<Integer> compose2() {
        HashSet<Integer> allSeries = new HashSet<>();
        HashSet<Integer> threeSeries = new HashSet<>();
        HashSet<Integer> twoSeries = new HashSet<>();
        HashSet<Integer> oneSeries = new HashSet<>();
        HashSet<Integer> allSames = new HashSet<>();
                                    ...
                                    for (int e4 = 0; e4 < 21; e4++) {
                                        ...
                                        for (byte p = 0; p < 34; p++)//考虑4顺
                                            if (!deserted[3].contains(p)) {//不能存在相应的对子
                                                count[0]++;
                                                newAlgorithms(allSeries, group, h34, p);//使用新的算法生成一副牌
                                            }
                                        ...
                                        if (!deserted[2].contains(s)) {
                                            ...
                                            for (byte p = 0; p < 34; p++)//考虑3顺+1刻
                                                if (!deserted[3].contains(p)) {
                                                    count[1]++;
                                                    newAlgorithms(threeSeries, group, h34, p);
                                                }
                                    ...
                                    for (byte s2 = 0; s2 < 34; s2++)
                                        if (!deserted[2].contains(s2)) {
                                            ...
                                            for (byte p = 0; p < 34; p++)
                                                if (!deserted[3].contains(p)) {
                                                    count[2]++;
                                                    newAlgorithms(twoSeries, group, h34, p);
                                                }
...
//                                        addLast(h34, deserted, s3, false);//考虑1顺+3刻
                                        ...
                                        for (byte p = 0; p < 34; p++)
                                            if (p != s && p != s2 && p != s3) {
                                                count[3]++;
                                                newAlgorithms(oneSeries, group, h34, p);
                                            }
                                        h34[s3] -= 3;
...
//                        removeLast(h34, deserted, s, false);
                                    ...
                                    if (s4 != s && s4 != s2 && s4 != s3)
                                        for (byte p = 0; p < 34; p++)
                                            if (p != s && p != s2 && p != s3 && p != s4) {//考虑全刻
                                                count[4]++;
                                                newAlgorithms(allSames, group, h34, p);
                                            }
            }
    ...
    private static void newAlgorithms(HashSet<Integer> allSeries, char[] group, byte[] h34, byte p) {
        h34[p] += 2;
...
//                group[n++] = array[i];
    ...
    private static void removeLast(byte[] h34, LinkedList[] deserted, int firstKey, boolean series) {
    ...
    private static void addLast(byte[] h34, LinkedList[] deserted, int firstKey, boolean series) {
    ...
    public static void main(String[] args) {
        ...
        HashSet<Integer> strings = compose2();
        ...
        System.out.println();
        for (Integer shape : strings) System.out.println(shape);
